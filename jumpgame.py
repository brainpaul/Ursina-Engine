...
app = Ursina()
ground = Entity(model= 'plane', texture= 'assets/grass.png', collider= 'mesh', scale= (100, 1, 100))
goal = Entity(model = 'cube', color= color.gold, texture= 'brick', position= (0, 11, 55), scale= (10, 1, 10), collider= 'box')
...
for i in range(10):
    r = uniform(-2,2)
    block = Entity(model= 'cube', color= color.brown, texture= 'brick', position= (r, 1+i, 3+i*5), scale= (3, 0.5, 3), collider= 'box')
    if r < 0:
        directions.append(1)
    else:
        directions.append(-1)
